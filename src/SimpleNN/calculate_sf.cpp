...
extern "C" int calculate_sf(double** cell, double** cart, double** scale,
                            ...
                            double** symf, double** dsymf, double* weights) {
    ...
    for (int ii=0; ii < cal_num; ++ii) {
        ...
        for (int j=0; j < nneigh; ++j) {
            ...
            for (int s=0; s < nsyms; ++s) {
                ...
                else if (params_i[s][0] == 22) {    //&& (params_i[s][1] == nei_list_i[j*2])) { // FIXME:
                    precal[0] = cutf(rRij / params_d[s][0]);
                    precal[1] = dcutf(rRij, params_d[s][0]);

                    symf[ii][s] += G2(rRij, precal, params_d[s], dradtmp)*weights[nei_list_i[j*2]-1]; // FIXME: index
                    tmpd[0] = dradtmp*vecij[0]*weights[nei_list_i[j*2]-1];
                    tmpd[1] = dradtmp*vecij[1]*weights[nei_list_i[j*2]-1];
                    tmpd[2] = dradtmp*vecij[2]*weights[nei_list_i[j*2]-1];
                ...
                for (int s=0; s < nsyms; ++s) {
                    if ((params_i[s][0] == 4) && 
                       (((params_i[s][1] == nei_list_i[j*2]) && (params_i[s][2] == nei_list_i[k*2])) || 
                        ...
                        symf[ii][s] += G4(rRij, rRik, rRjk, powtwo[s], precal, params_d[s], dangtmp)*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);

                        tmpd[0] = dangtmp[0]*vecij[0]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        tmpd[1] = dangtmp[0]*vecij[1]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        tmpd[2] = dangtmp[0]*vecij[2]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        tmpd[3] = dangtmp[1]*vecik[0]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        tmpd[4] = dangtmp[1]*vecik[1]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        tmpd[5] = dangtmp[1]*vecik[2]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        tmpd[6] = dangtmp[2]*vecjk[0]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        tmpd[7] = dangtmp[2]*vecjk[1]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        tmpd[8] = dangtmp[2]*vecjk[2]*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
                        ...
                        symf[ii][s] += G5(rRij, rRik, powtwo[s], precal, params_d[s], dangtmp)*sqrt(weights[nei_list_i[j*2]-1]*weights[nei_list_i[k*2]-1]);
