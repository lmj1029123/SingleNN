...
import sys
import time
def train(train_dict, val_dict, model, model_path, opt_method, optimizer, E_coeff, F_coeff, epoch, val_interval, n_val_stop, convergence, is_force, logfile):
...
def evaluate(data_dict, model_path, E_coeff, F_coeff, is_force):
    SSE = torch.nn.MSELoss(reduction='sum')
    SAE = torch.nn.L1Loss(reduction='sum')
